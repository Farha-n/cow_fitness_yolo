...
## Quick Start (Current Folder Layout)

This repository is organized with top-level folders: `backend/`, `frontend/`, and `models/`.

From repository root:

```bash
# Start API
F:/Cow/cow_fitness_yolo/.venv/Scripts/python.exe -m uvicorn backend.src.api:app --host 0.0.0.0 --port 8000 --reload

# Run detector inference script
F:/Cow/cow_fitness_yolo/.venv/Scripts/python.exe backend/src/infer.py path/to/image.jpg
...
Model defaults used by backend scripts/API now point to:

- `models/trained/cow_yolos_best.pt`
- `models/trained/cow_fitness_cls.pt`
- `models/trained/cattle_species_cls.pt` (optional)
- `models/trained/cow_breed_cls.pt` (optional)
...
## Using Python (from repo root)

```bash
F:/Cow/cow_fitness_yolo/.venv/Scripts/python.exe backend/src/infer.py path/to/image.jpg
```

The detector uses `models/trained/cow_yolos_best.pt` by default.

## 🏋️ Training (from repo root)

```bash
F:/Cow/cow_fitness_yolo/.venv/Scripts/python.exe backend/src/train.py
```

Training outputs are written under `backend/runs/`.
